...
-->
  ...
  <xsl:template mode="mode-iso19115-3"
    ...
    <xsl:variable name="xpath" select="gn-fn-metadata:getXPath(.)"/>
		...
		<!-- Joseph added for eCat ID' label -->
		<xsl:variable name="eCatIdNode">
			<label>eCat ID</label>
		</xsl:variable>
		<xsl:for-each select="cit:CI_Citation/cit:identifier/mcc:MD_Identifier/mcc:code">

    	<xsl:call-template name="render-element">
      	<xsl:with-param name="label" select="$eCatIdNode"/>
 ...
 <!-- <xsl:template mode="mode-iso19115-3"
  ...
  <xsl:template mode="mode-iso19115-3"
                match="*[
                        *[1]/name() = $editorConfig/editor/tableFields/table/@for and
                        ...
                        $isFlatMode]"
                priority="2000">
    <xsl:call-template name="iso19115-3-table"/>
  </xsl:template>
  ...
  <!-- Define table layout -->
  <xsl:template name="iso19115-3-table">
